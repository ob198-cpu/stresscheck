...
const FORM_DESCRIPTION = [
  ...
  "",
  "結果通知や医師面接指導の申出時に会社へ伝わる内容は、実施前の説明文・実施規程に従って取り扱います。"
].join("\n");
...
function createStressCheckGoogleForm() {
  ...
  form.addSectionHeaderItem()
    .setTitle("回答前の確認")
    .setHelpText("本人専用URLまたは案内文に記載された受検者IDで回答してください。氏名やメールアドレスは、このフォームでは原則として収集しません。");
  ...
  const spreadsheet = SpreadsheetApp.create(`${FORM_TITLE} 回答`);
  ...
  const result = {
    ...
    questionCount: 57,
    note: "フォームと回答スプレッドシートは作成者アカウントのDriveに作成されています。会社担当者には編集権限・回答スプレッドシート権限を付けないでください。"
  };
