...
const downloadInterviewFollowupCsv = document.querySelector("#downloadInterviewFollowupCsv");
...
function buildCompanyDisclosureCsv(rows = googleImportRows) {
  ...
  const output = [
    ["区分", "集団名", "人数", "企業共有可否", "高ストレス者割合", "総合健康リスク", "量-コントロール", "職場の支援", "確認欄", "備考"],
    ...(summary.overall ? [[
      "会社全体",
      summary.overall.label,
      summary.overall.count,
      "共有可",
      summary.overall.highStressRate,
      summary.overall.healthRisk?.total || "",
      summary.overall.healthRisk?.loadControl || "",
      summary.overall.healthRisk?.support || "",
      "",
      "10人以上のため企業向け集団分析に表示",
    ]] : [[
      "会社全体",
      "会社全体",
      summary.scoreableRows,
      "共有不可",
      "",
      "",
      "",
    ...
    ...summary.visibleGroups.map((group) => [
      ...
      group.count,
      "共有可",
      group.highStressRate,
      group.healthRisk?.total || "",
      group.healthRisk?.loadControl || "",
      group.healthRisk?.support || "",
      "",
      "10人以上のため企業向け集団分析に表示",
    ]),
    ...summary.suppressedGroups.map((group) => [
      "非表示集団",
      group.label,
      group.count,
      "共有不可",
      ...
      "",
      "",
      "10人未満のため個人特定防止として数値非表示",
    ]),
  ];
  return output.map((line) => line.map(csvCell).join(",")).join("\r\n");
}

function buildInterviewFollowupCsv(rows = googleImportRows) {
  const highStressRows = rows
    .map((row) => ({ row, analysis: buildMhlwIndividualAnalysis(row) }))
    .filter(({ analysis }) => analysis.canScore && analysis.highStress);
  const output = [
    [
      "CSV行",
      "受検者ID",
    ...
    ...highStressRows.map(({ row, analysis }) => [
      row.sourceRowNumber || "",
      row.respondentId || row.participantCode || "",
      row.personName || "",
      row.kanaName || "",
      ...
      "",
      "",
      "実施者・実施事務従事者のみ管理。本人同意なしに会社担当者へ個人結果を共有しない。",
    ]),
  ];
  return output.map((line) => line.map(csvCell).join(",")).join("\r\n");
}
...
function buildRetentionManifestCsv(rows = googleImportRows) {
  ...
  const files = [
    ...
    ["企業共有チェックCSV", names.companyDisclosure, hasOperation("企業共有チェックCSV") ? "保存済み" : "未保存"],
    ["面接指導対応CSV", names.interviewFollowup, hasOperation("面接指導対応CSV") ? "保存済み" : "未保存"],
    ...
    ["実施完了チェックCSV", names.completionPackage, hasOperation("実施完了チェックCSV") ? "保存済み" : "未保存"],
  ];
  const output = [
    ["項目", "値"],
    ["実施ID", currentRunId || ""],
...
function buildCompletionPackageCsv(rows = googleImportRows) {
  ...
  const output = [
    ...
    [2, "修正", "修正リストCSVで不足情報・判定不可行を潰す", names.fixList, fixListCount ? `${fixListCount}件の補完候補` : "修正対象なし", fixListCount ? (hasOperation("修正リストCSV") ? "済" : "未") : "対象なし", ""],
    [3, "本人分析", "個人分析CSVを保存", names.individualCsv, `判定可能 ${scoreableCount}件`, hasOperation("個人分析CSV保存") ? "済" : "未", ""],
    [4, "本人通知", "本人向け結果を開いてPDF保存", names.personal, scoreableCount ? `対象 ${scoreableCount}件` : "対象なし", scoreableCount ? (hasOperation("本人向け結果") ? "済" : "未") : "対象なし", ""],
    [5, "本人通知", "本人配布チェックCSVでPDF保存・通知日を管理", names.personalDelivery, "本人へ直接通知。会社担当者へ個人結果を渡さない", hasOperation("本人配布チェックCSV") ? "済" : "未", ""],
    [6, "面接指導", "高ストレス者の申出案内・面接指導対応を管理", names.interviewFollowup, highStressCount ? `対象 ${highStressCount}件` : "対象なし", highStressCount ? (hasOperation("面接指導対応CSV") ? "済" : "未") : "対象なし", ""],
    [7, "企業共有", "企業向け集団分析を開いてPDF保存", names.company, groupAnalysis.overall || groupAnalysis.visibleGroups.length ? `表示集団 ${groupAnalysis.visibleGroups.length}件` : "10人以上の集団なし", hasOperation("企業向け集団分析") ? "済" : "未", ""],
    [8, "企業共有", "企業共有チェックCSVで共有可否を確認", names.companyDisclosure, `非表示集団 ${groupAnalysis.suppressedGroups.length}件`, hasOperation("企業共有チェックCSV") ? "済" : "未", ""],
...
function guideItem(label, done, detail, why = "", critical = false) {
  return `
    <div class="requirement-item ${done ? "ok" : "todo"} ${critical && !done ? "critical" : ""}">
      <strong>${done ? "OK" : critical ? "重大" : "要確認"}</strong>
      <span>${escapeHtml(label)}<em>${escapeHtml(detail)}</em>${why ? `<small>${escapeHtml(why)}</small>` : ""}</span>
    </div>
  `;
}

function nextRequirementAction(states) {
  const next = states.find((item) => !item.done && item.next);
...
function requirementReadiness(states) {
  ...
  const criticalRemainingCount = states.filter((item) => item.critical && !item.done).length;
  ...
  if (remainingCount === 0) {
    return {
      ...
      totalCount,
      remainingCount,
      criticalRemainingCount,
    };
  }
  if (criticalRemainingCount) {
    return {
      level: "critical",
      label: "重大未完了あり",
      detail: `重大な未完了が ${criticalRemainingCount}件あります。本番出力・配布前に先に確認してください。`,
      percent,
      doneCount,
      totalCount,
      remainingCount,
  ...
  return `
    <div class="requirements-readiness ${escapeHtml(readiness.level)}">
      <div>
        ...
        <span>${escapeHtml(readiness.doneCount)} / ${escapeHtml(readiness.totalCount)} 完了・重大残 ${escapeHtml(readiness.criticalRemainingCount || 0)}件</span>
      </div>
...
function renderReadinessSummary(readiness, nextAction) {
  ...
  readinessSummary.innerHTML = `
    <div class="readiness-summary-main ${escapeHtml(readiness.level)} ${escapeHtml(modeClass)}">
      ...
      <span>${escapeHtml(currentRunMode)} / ${escapeHtml(readiness.percent)}% / 残り ${escapeHtml(readiness.remainingCount)}件 / 重大 ${escapeHtml(readiness.criticalRemainingCount || 0)}件</span>
...
function renderRequirementsGuide(rows = googleImportRows) {
  ...
  const items = states.map((item) => guideItem(item.label, item.done, item.detail, item.why, item.critical));
...
function buildReadinessCsv(rows = googleImportRows) {
  ...
  const output = [
    ...
    [],
    ["ナビ項目", "項目", "状態", "重要度", "詳細", "確認観点", "次にやること"],
  ];
  snapshot.states.forEach((item) => {
    output.push(["ナビ項目", item.label, item.done ? "OK" : "要確認", item.critical ? "重大" : "通常", item.detail, item.why || "", item.done ? "" : `${item.next?.label || ""} ${item.next?.detail || ""}`.trim()]);
  });
  return output.map((line) => line.map(csvCell).join(",")).join("\r\n");
}
...
function recommendedFileNames(rows = googleImportRows) {
  ...
  return {
    ...
    companyDisclosure: fileNameWithRunId("company-disclosure-check", "csv"),
    ...
    completionPackage: fileNameWithRunId("stress-check-completion-package", "csv"),
  };
}
...
function renderCompletionChecklist(rows = googleImportRows) {
  ...
  const items = [
    ...
    statusItem("企業共有チェックCSV", hasOperation("企業共有チェックCSV"), `表示 ${groupAnalysis.visibleGroups.length}件 / 非表示 ${groupAnalysis.suppressedGroups.length}件`),
    ...
    statusItem("実施完了チェックCSV", hasOperation("実施完了チェックCSV"), "保存漏れの最終確認"),
  ];
  completionChecklist.innerHTML = `
    <strong>完了状況</strong>
    ...
    <div class="recommended-files">
      ...
      <span>${escapeHtml(names.companyDisclosure)}</span>
...
function renderIndividualAnalysisPreview(rows) {
  ...
  if (!rows.length) {
    ...
    if (downloadCompanyDisclosureCsv) downloadCompanyDisclosureCsv.disabled = true;
    if (downloadInterviewFollowupCsv) downloadInterviewFollowupCsv.disabled = true;
  ...
  if (downloadInterviewFollowupCsv) downloadInterviewFollowupCsv.disabled = !highStressCount;
...
function requireProductionCsvConfirmation(outputLabel) {
  if (currentRunMode !== "本番CSV未確認") return true;
  setGoogleImportMessage(`${outputLabel}の前に、選択したCSVが本番のGoogleフォーム回答CSVであることを確認してください。`, "error");
  focusRequirementTarget("#productionCsvConfirm");
  return false;
}
...
function handleDownloadCompletionPackageCsv() {
  if (!googleImportRows.length) {
    setGoogleImportMessage("先にCSVを確認してください。", "error");
    return;
  }
  if (!requireProductionCsvConfirmation("実施完了チェックCSV保存")) return;
  const names = recommendedFileNames(googleImportRows);
  downloadTextFile(names.completionPackage, `\uFEFF${buildCompletionPackageCsv(googleImportRows)}`, "text/csv;charset=utf-8");
  setGoogleImportMessage(`実施完了チェックCSVを保存しました。保存漏れ・配布漏れの最終確認に使ってください。実施ID: ${currentRunId || "-"}`, "success");
...
function handleDownloadCompanyDisclosureCsv() {
  if (!googleImportRows.length) {
    setGoogleImportMessage("先にCSVを確認してください。", "error");
    return;
  }
  ...
  setGoogleImportMessage(`企業共有チェックCSVを保存しました。表示可能集団 ${summary.visibleGroups.length}件、非表示集団 ${summary.suppressedGroups.length}件を確認できます。`, "success");
  addOperationLog("企業共有チェックCSV保存", { visibleGroups: summary.visibleGroups.length, suppressedGroups: summary.suppressedGroups.length });
}

function handleDownloadInterviewFollowupCsv() {
  if (!googleImportRows.length) {
    setGoogleImportMessage("先にCSVを確認してください。", "error");
    return;
  }
  ...
  if (!confirmOutputReadiness("面接指導対応CSV", { requireLegalChecks: true })) return;
  const names = recommendedFileNames(googleImportRows);
  downloadTextFile(names.interviewFollowup, `\uFEFF${buildInterviewFollowupCsv(googleImportRows)}`, "text/csv;charset=utf-8");
  setGoogleImportMessage(`面接指導対応CSVを保存しました。高ストレス者 ${highStressCount}件の申出案内・面接指導対応を実施者側で管理してください。`, "success");
  addOperationLog("面接指導対応CSV保存", { highStressCount });
}
...
downloadInterviewFollowupCsv.addEventListener("click", handleDownloadInterviewFollowupCsv);
...
googleCsvFile.addEventListener("change", () => {
  ...
  downloadCompanyDisclosureCsv.disabled = true;
  downloadInterviewFollowupCsv.disabled = true;
